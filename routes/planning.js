...
router.post("/areNotFixed", (req, res) => {
  const { userToken, currentTripId } = req.body;
  if (!userToken || !currentTripId) {
    return res.json({ result: false, error: "Token or TripID are missing" });
  }
  Trip.findById(currentTripId)
    .then((trip) => {
      if (!trip) {
      ...
      .then((populatedTrip) => {
        console.log('Dans le then populatetrip', )
  ...
  Trip.updateOne(filter, update)
    .then((data) => {
      ...
      if (data.modifiedCount > 0) {
        ...
        Trip.findOne({ "activities._id": activityId }).then((trip) => {
          const updatedActivity = trip.activities.find((activity) =>
